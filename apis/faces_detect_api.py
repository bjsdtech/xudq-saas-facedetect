...
from commons.response_result import face_detect_return
...
async def faces_detect(version: str, picType: str, transId: str, timestamp: str, nonce: str,
    ...
    if not picType.strip():
        logger.error("picType para is empty.")
        logger.info("(transid:" + transId + ")face detect end.")
        return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_PICTYPE_EMPTY.value, 4, "picType para is empty.")

    if not transId.strip():
        logger.error("transId is empty.")
        logger.info("(transid:" + transId + ")face detect end.")
        return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_TRANSID_EMPTY.value, 4, "transId is empty.")

    if not timestamp.strip():
        logger.error("timestamp para is empty.")
        logger.info("(transid:" + transId + ")face detect end.")
        return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_TIMESTAMP_EMPTY.value, 4, "timestamp para is empty.")

    if not nonce.strip():
        logger.error("nonce para is empty.")
        logger.info("(transid:" + transId + ")face detect end.")
        return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_NONCE_EMPTY.value, 4, "nonce para is empty.")

    if file is None:
        logger.error("image file para is empty.")
        logger.info("(transid:" + transId + ")face detect end.")
        return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_FILE_NONE.value, 4, "image file para is empty.")
    ...
    if not file:
        status = 3
        face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_FILE_NONE.value, status, "para(file) error.")
    else:
    ...
    if len(result_split) < 2:
        ...
        logger.info("(transid:" + transId + ")face detect end.")
        status = 4
        return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_IMAGE_FORMAT.value, status, "image name format error.")
    ...
    if not image_type_check(image_filetype):
        ...
        logger.info("(transid:" + transId + ")face detect end.")
        status = 4
        return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_IMAGE_TYPE.value, status, "image type error.")
    ...
    if "v1" != version:  # 版本v1 人脸检测
        ...
        logger.info("(transid:" + transId + ")face detect end.")
        status = 2
        return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_VAR_NO_SUPPORT.value, status, "version must be v1")
    ...
    if face_number > 0:
        ...
        try:
            ...
            status = 2
            return face_detect_return(transId, ERROR_CODE.FD_CHK_ERROR_DB_EXCEPT, status, "database except.")

        status = 1
        result = face_detect_return(transId, ERROR_CODE.FD_CHK_SUCCESS.value, status, "success", inner_trans_id, face_number, face_locations)
        ...
        status = 1
        result = face_detect_return(transId, ERROR_CODE.FD_CHK_SUCCESS.value, status, "success")
